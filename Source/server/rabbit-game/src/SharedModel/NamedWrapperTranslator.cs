	...
	public class NamedWrapperTranslator : IProtocolTranslator
	{
		...
		public Message ToMessage(string payload)
		{
			...
			try
			{
				...
				else if (wrappedMsg.name == MessageType.BuildUnit.ToString())
				{
				...
				else if (wrappedMsg.name == MessageType.LeaveGame.ToString())
				{
					return serializer.ToObj<LeaveGameMessage>(wrappedMsg.payload);
				}
				else
				{
